...
class QCOMComputeQueue(HWQueue):
  ...
  def kernargs(self, call:UOp, prg:UOp, data:QCOMProgramData) -> UOp:
    kargs = get_call_kernel_args(call, prg)
    # images get texture/ibo slots, every other arg (buffer address or value) keeps the signature order
    plain = [a.getaddr(self.devs) if sig[4] else a.ccast(sig[2]) for a, sig in kargs if not (sig[4] and is_image_shape(sig[3]))]
    uavs = [(sig[2], sig[3], a) for a, sig in kargs if sig[4] and is_image_shape(sig[3])]
    ...
    if data.NIR:
      args += layout_args(plain, data.buf_off)
      if data.wgsz != 0xfc: args += layout_args(list(prg.arg.local_size), data.wgsz * 4)
    else: args += list(zip(data.buf_offs, plain))
