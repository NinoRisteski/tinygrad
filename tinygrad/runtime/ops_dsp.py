...
class DSPRenderer(ClangRenderer):
  ...
  def _render_entry(self, function_name:str, bufs:list[tuple[str,tuple[UOp,bool]]]) -> str:
    ...
    # buffer offsets and fds are passed per buffer, the params can interleave buffers and values
...
class DSPProgram(Program['DSPDevice']):
  ...
  def __call__(self, *bufs, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False, **kw):
    ...
    for i,(a,(_,_,dt,_,is_buffer)) in enumerate(TinyELF.runtime_args(self.signature, bufs, vals)):
      struct.pack_into('i' if is_buffer else unwrap(dt.fmt), var_vals_mv, i*8, a.size if is_buffer else a)
  ...
  def __call__(self, *bufs, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False, **kw):
    with tempfile.NamedTemporaryFile(suffix=".out") as dsp_lib:
      ...
      proc = subprocess.run(["qemu-hexagon-static", *(['-strace'] if DEBUG >= 5 else []), dsp_lib.name],
        input=b''.join(bytes(to_mv(x.va_addr, x.size)) if is_buffer else struct.pack(unwrap(dt.fmt), x)
                       for x,(_,_,dt,_,is_buffer) in TinyELF.runtime_args(self.signature, bufs, vals)),
