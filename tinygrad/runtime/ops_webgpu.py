...
class WebGPUProgram(Program['WebGpuDevice']):
  ...
  def __call__(self, *bufs:webgpu.WGPUBuffer, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    args = TinyELF.runtime_args(self.signature, bufs, vals)
    bind_entries = (webgpu.WGPUBindGroupLayoutEntry * (1+len(args)))(
      bgl_entry(0, 'Uniform'), *(bgl_entry(i+1, 'Storage' if sig[4] else 'Uniform') for i,(_,sig) in enumerate(args)))
    ...
    bindings = (webgpu.WGPUBindGroupEntry * (1+len(bufs)+len(vals)))(bg_entry(0, float('inf')), *(bg_entry(i+1, x) for i,(x,_) in enumerate(args)))
