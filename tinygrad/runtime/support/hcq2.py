...
from tinygrad.engine.realize import get_call_arg_uops, get_call_bufs, get_call_name, get_call_outs_ins, get_call_written_bufs
...
def layout_args(args:Sequence[UOp|int], offset:int=0) -> list[tuple[int, UOp]]:
  ...
  return [(offset + o, w) for (o, _), w in zip(TinyELF.iter_sig(tuple((None, i, w.dtype, (), False) for i, w in enumerate(words))), words)]
...
def _finalize_batch(ctx:BatchCtx, skip_wait:bool=False) -> UOp:
  ...
  args = [[unwrap_lane(b)[:2] for b in get_call_bufs(c)] for c, _, _ in ctx.batch]
