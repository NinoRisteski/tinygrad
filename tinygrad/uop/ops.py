...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def _shape(self) -> tuple[sint, ...]|None:
    ...
    if self.op in GroupOp.Movement.union({Ops.MULTI, Ops.REDUCE_AXIS, Ops.WMMA}):
      ...
      match self.op:
        ...
        case Ops.PAD:
          if len(ps) != len(self.marg) or not all(resolve(s >= 0) for s in self.marg): raise ValueError(f"invalid pad output shape {self.marg}")
          return tuple(ssimplify(s) for s in self.marg)
        case Ops.SHRINK:
          if len(ps) != len(self.marg) or not all(resolve(s >= 0) for s in self.marg): raise ValueError(f"invalid shrink output shape {self.marg}")
          return tuple(ssimplify(s) for s in self.marg)
        case Ops.FLIP:
  ...
  def marg(self):
    match self.op:
      ...
      case Ops.PAD | Ops.SHRINK: return tuple(self.src[1].sgep(i) for i in range(self.src[1].dtype.count))
  ...
  def _mop(self, op:Ops, arg, same_shape_noop:bool=False) -> UOp:
    match op:
      ...
      case Ops.PAD | Ops.SHRINK: src_args = [arg]
